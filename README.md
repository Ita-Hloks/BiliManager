...
- 搜索结果过滤：按标题规则隐藏、模糊或降权视频。
- 播放页个性化：隐藏相关推荐、播放器广告，并控制相关推荐自动播放。
- 观看计时：记录每日观看时长和最近观看视频。
- 主题与背景：支持浅色、深色和跟随系统主题，并可设置自定义背景。
...
导入前建议先导出当前数据作为备份。扩展也兼容直接导入旧版的设置 JSON；纯文本内容会按换行分隔，作为标题过滤规则导入。
